...
`extract` is like `separate` but uses a `regex` instead of `sep`, `sep` default accepts a regex expression, but it allows numeric whereas I'm guessing
...
tibble(x = c("a,b,c", "d,e,f", "h,i,j", "k,l,m")) %>% 
  separate(x, c("one", "two", "three"), remove = FALSE) %>% 
  unite(one, two, three, col = "x2", sep = ",", remove = FALSE) %>% 
  extract(x, c("a", "b", "c"), "([a-z]+),([a-z]+),([a-z]+)")
```

    ## # A tibble: 4 x 7
    ##       a     b     c    x2   one   two three
...
![](ch12_files/figure-markdown_github/unnamed-chunk-21-1.png)
...
![](ch12_files/figure-markdown_github/unnamed-chunk-21-2.png)
...
![](ch12_files/figure-markdown_github/unnamed-chunk-21-3.png)
...
![](ch12_files/figure-markdown_github/unnamed-chunk-21-4.png)
...
![](ch12_files/figure-markdown_github/unnamed-chunk-21-5.png)
...
![](ch12_files/figure-markdown_github/unnamed-chunk-21-6.png)
