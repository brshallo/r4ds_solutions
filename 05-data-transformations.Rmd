...
```{r setup05, include=FALSE, message=FALSE, warning=FALSE}
...
It can be a hassle naming the values explicitly. `quantile`'s default `probs` argument value is `r seq(0, 1, 0.25)`. Rather than needing to type the `delays_q` values `list(c('0%', '25%', '50%', '75%', '100%'))` you could have generated the values of these names dynamically using the `map` function in the `purrr` package (see chapter on iteration) with example  for this by passing the `names` function over each value in `delays_val`.  
