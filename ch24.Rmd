...
To fix challenges you may need to use other types of transformations like box-cox, rescaling, centering, etc.^[The point being that log-transforms are not a panacea for any fix.] or other modeling techniques.
...
log_notes_df3 %>% 
  ...
  geom_hex()+
  geom_abline(aes(intercept = coef(mod_exp_dist_flipped)[[1]], slope = coef(mod_exp_dist_flipped)[[2]]), colour = "red")+
  lims(x = c(0, 30), y = c(0, 30))
...
* Note that when using the origin as the predictor we have the coefficient centered at ~ 1, whereas when the predictor is rand_noise, coefficients are closer to 0.8
